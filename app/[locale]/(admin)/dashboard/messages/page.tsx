...
interface MessageData {
  ...
  status: string;
  created_at: string;
  car: { id: string; brand: { name: string } | null; model: { name: string } | null } | null;
}

function StatusBadge({ status, t }: { status: string; t: any }) {
  const variants = { new: 'destructive', read: 'default', replied: 'success' } as const;
  const labels = { new: t('status.new'), read: t('status.read'), replied: t('status.replied') };
...
export default function MessagesPage() {
  ...
  const fetchMessages = async () => {
    ...
    let query = supabase.from('contact_messages').select(`id, full_name, email, phone, message, status, created_at, car:cars(id, brand:brands(name), model:models(name))`, { count: 'exact' }).range(from, to).order('created_at', { ascending: false });
              ...
              messages.map((msg) => (
                <TableRow key={msg.id}>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {msg.car ? (
                      <p className="text-sm text-neutral-600">{msg.car.brand?.name} {msg.car.model?.name}</p>
                    ) : (
